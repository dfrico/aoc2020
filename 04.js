...
const validation = {
  ...
  hgt: (val) => {
    if (!val) return false;
    if (val.includes("cm")) {
      const num = +val.split("cm")[0];
      return num >= 150 && num <= 193;
    }
    if (val.includes("in")) {
      const num = +val.split("in")[0];
  ...
  reqs.forEach((key) => {
    if (i < 20)
      console.log(
        key,
        passport[key],
        validation[key].toString(),
        validation[key](passport[key])
      );
    if (Object.keys(passport).includes(key) && validation[key](passport[key]))
      validKeys += 1;
  });
  if (validKeys === reqs.length) counter += 1;
});
